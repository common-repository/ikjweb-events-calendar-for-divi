=== IKJ Events Calendar For Divi ===
Contributors: ilenejohnson
Tags: Divi, Tribe, Events
Donate link: https://www.paypal.com/donate/?hosted_button_id=JXZSC8T8BK7ME
Requires at least: 5.5
Tested up to: 6.4.2
Requires PHP: 7.4
Stable tag: 1.0.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Extension of Tribe Events Calendar - shows recent Events


== Description ==

IKJ Events Calendar For Divi is a WordPress plugin extends the Tribe Events Calendar.  

== Installation ==

1. Upload the plugin folder to your /wp-content/plugins/ folder.
2. Go to the **Plugins** page and activate the plugin.
3. In the DIVI builder, selected Insert module and chose either "Blog Events for Divi - Tribe Ext" or "Upcoming Events for Divi - Tribe Ext". 

== Screenshots ==

1.  Insert Tribe Events for Divi Blog Extension
2.  Blog Events for Divi Content 
3.  Blog Events For Divi Style Settings
4.  Blog Events for Divi Various Title Settings
5.  Blog Events for Divi Screen shot with description 
10. Insert Upcoming Events for Divi - Tribe Ext Settings
6.  Upcoming Events for Divi - Content
7.  Upcoming Events for Divi - Tribe Ext Settings Style Settings
8.  Upcoming Events for Divi - Tribe Ext Settings Title Text Settings
9.  Upcoming Events for Divi - Tribe Ext  Screen shot with descriptions 

== Frequently Asked Questions == 

= Are other plugins required?  

Yes, you will need the Events Calendar by Tribe as this plugin is an extention and either have a DIVI theme or the DIVI plugin

== Changelog ==

1.0  Initial submission


== Upgrade Notice ==

= 1.0 =

* First Public Release